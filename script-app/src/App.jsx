...
function Calendar({ userId }) {
  ...
  const shareCalendar=async()=>{
    ...
    try{
      ...
      const{error}=await sb.from("shared_calendars").upsert({id,user_id:userId,data:evtsRef.current,owner_email:email,updated_at:new Date().toISOString()},{onConflict:"id"});
      if(error) throw error;
      const link="https://script-sable.vercel.app/#share/"+id;
      if(navigator.share){ await navigator.share({title:"My Script Calendar",url:link}); } else { await navigator.clipboard.writeText(link); }
      setShareState("copied"); setTimeout(()=>setShareState("idle"),3000);
    }catch(e){
      if(e?.name==="AbortError"){setShareState("idle");return;}
      console.error(e); setShareState("error"); setTimeout(()=>setShareState("idle"),3000);
    }
  };
...
function Settings({ user, dark, setDark, onClose }) {
  ...
  const [err,setErr]=useState("");
  const [loading,setLoading]=useState(false);
  const bg=dark?"#1A0000":"#fff", bg2=dark?"#2A0A0A":"#F5F0EE", bdr=dark?"#4A1A1A":C.bd, txt=dark?"#F2F2F7":C.k, sub=dark?"#8E8E93":C.g;
...
export default function Script() {
  ...
  const D=dark?{pageBg:"#1A0000",headerBg:"#2A0A0A",border:"#4A1A1A",text:"#F2F2F7",sub:"#8E8E93"}:{pageBg:"#F5F0EE",headerBg:"#fff",border:C.bd,text:C.k,sub:C.g};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100svh",background:D.pageBg,fontFamily:"'Nunito',sans-serif",overflow:"hidden",paddingTop:"env(safe-area-inset-top)"}}>
      {dark&&<style>{`input,select,textarea{background:#2A0A0A !important;color:#F2F2F7 !important;border-color:#4A1A1A !important;}input::placeholder,textarea::placeholder{color:#886666 !important;}`}</style>}
