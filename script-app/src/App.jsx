...
function Login({ onLogin }) {
  ...
  const II = {width:"100%",padding:"13px 16px",borderRadius:12,border:"1.5px solid rgba(255,255,255,.15)",background:"rgba(255,255,255,.08)",color:"#fff",fontSize:15,fontFamily:"inherit",outline:"none",boxSizing:"border-box"};
  return (
    <div style={{minHeight:"100svh",background:C.r,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 16px",fontFamily:"'DM Sans',sans-serif",overflowY:"auto"}}>
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:12,marginBottom:12}}>
            <span style={{fontFamily:"'Nunito',sans-serif",fontSize:52,fontWeight:900,color:"#fff",lineHeight:1}}>Script</span>
            <ScrollIcon sz={48} white={true}/>
          </div>
          <div style={{color:"rgba(255,255,255,.5)",fontSize:13,letterSpacing:2,fontWeight:600}}>Personal Organizer</div>
        </div>
        <div style={{background:"rgba(255,255,255,.12)",border:"1px solid rgba(255,255,255,.2)",borderRadius:22,padding:"26px 22px"}}>
          {/* Mode tabs */}
          {mode !== "reset" && (
            <div style={{display:"flex",background:"rgba(255,255,255,.06)",borderRadius:12,padding:4,marginBottom:24}}>
              {[["in","Sign In"],["up","Create Account"]].map(([m,l])=>(
                <button key={m} onClick={()=>{setMode(m);setErr("");setInfo("");}} style={{flex:1,padding:"10px 8px",borderRadius:9,border:"none",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit",background:mode===m?C.r:"transparent",color:mode===m?"#fff":"rgba(255,255,255,.35)"}}>{l}</button>
              ))}
            </div>
          )}
          {mode === "reset" && (
            <div style={{marginBottom:20}}>
              <button onClick={()=>{setMode("in");setErr("");setInfo("");}} style={{background:"none",border:"none",color:"rgba(255,255,255,.6)",fontSize:13,cursor:"pointer",fontFamily:"inherit",padding:0,display:"flex",alignItems:"center",gap:4}}>← Back to Sign In</button>
              <div style={{color:"#fff",fontWeight:700,fontSize:17,marginTop:10}}>Reset Password</div>
              <div style={{color:"rgba(255,255,255,.5)",fontSize:13,marginTop:4}}>We'll send a reset link to your email.</div>
            </div>
          )}

          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div>
              <div style={{color:"rgba(255,255,255,.4)",fontSize:10,letterSpacing:2,textTransform:"uppercase",marginBottom:7,fontWeight:700}}>Email</div>
            ...
            {mode !== "reset" && (
              <div>
                <div style={{color:"rgba(255,255,255,.4)",fontSize:10,letterSpacing:2,textTransform:"uppercase",marginBottom:7,fontWeight:700}}>Password</div>
          ...
          {mode === "up" && (
            <div style={{marginTop:16,background:"rgba(0,0,0,.15)",borderRadius:12,padding:"14px 16px"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                <button onClick={()=>setAgreed(!agreed)} style={{width:20,height:20,borderRadius:5,border:"2px solid "+(agreed?"#fff":"rgba(255,255,255,.4)"),background:agreed?"#fff":"transparent",flexShrink:0,marginTop:1,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              {showDisc && (
                <div style={{marginTop:12,padding:"12px 14px",background:"rgba(0,0,0,.2)",borderRadius:10,fontSize:11,color:"rgba(255,255,255,.65)",lineHeight:1.7}}>
                  <div style={{fontWeight:800,color:"rgba(255,255,255,.9)",fontSize:12,marginBottom:6}}>⚠️ Data Disclaimer</div>
          ...
          {err  && <div style={{marginTop:14,color:"#ff8080",fontSize:13,textAlign:"center",background:"rgba(255,80,80,.1)",borderRadius:8,padding:"8px 12px"}}>{err}</div>}
          {info && <div style={{marginTop:14,color:"#a0ffb0",fontSize:13,textAlign:"center",background:"rgba(80,255,120,.08)",borderRadius:8,padding:"8px 12px"}}>{info}</div>}

          <button onClick={go} disabled={loading} style={{width:"100%",marginTop:18,padding:"14px",borderRadius:14,border:"none",background:"rgba(255,255,255,.18)",color:"#fff",fontWeight:800,fontSize:16,fontFamily:"'Nunito',sans-serif",cursor:"pointer",opacity:loading?0.6:1}}>
            {loading ? "Please wait..." : mode==="reset" ? "Send Reset Email" : mode==="in" ? "Sign In →" : "Create Account →"}
          </button>

          {mode === "in" && (
            <button onClick={()=>{setMode("reset");setErr("");setInfo("");}} style={{width:"100%",marginTop:12,background:"none",border:"none",color:"rgba(255,255,255,.4)",fontSize:12,cursor:"pointer",fontFamily:"inherit",textDecoration:"underline"}}>
              Forgot password?
            </button>
          )}
        </div>
        <p style={{textAlign:"center",color:"rgba(255,255,255,.35)",fontSize:11,marginTop:16,letterSpacing:1}}>Your data is saved to the cloud ☁️</p>
      </div>
    </div>
  );
}
...
function Chalk({ userId }) {
  ...
  useEffect(() => { dbLoad(userId,"chalk").then(v=>{ if(v!==null) setText(v); setLoaded(true); }); }, [userId]);

  const onChange = val => {
    setText(val);
    clearTimeout(timer.current);
    timer.current = setTimeout(async ()=>{ setSaving(true); await dbSave(userId,"chalk",val); setSaving(false); }, 800);
  };
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",borderRadius:10,background:"#6b8c52",boxShadow:"inset 0 0 60px rgba(0,0,0,.25)"}}>
      <div style={{flexShrink:0,background:"rgba(0,0,0,.22)",borderBottom:"2px solid rgba(0,0,0,.3)",padding:"8px 14px 9px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <span style={{color:"rgba(255,255,255,.35)",fontSize:12}}>{saving?"💾 saving...":"☁️ saved"}</span>
          <button onClick={()=>onChange("")} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.15)",borderRadius:6,color:"rgba(255,255,255,.65)",padding:"3px 13px",fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>Erase</button>
        </div>
      </div>
      <div style={{flex:1,position:"relative",overflow:"hidden"}}>
        {!loaded && <div style={{position:"absolute",inset:0,background:"#6b8c52",display:"flex",alignItems:"center",justifyContent:"center"}}><Spinner msg="Loading..."/></div>}
        <textarea value={text} onChange={e=>onChange(e.target.value)} placeholder="Write anything here..."
...
function Money({ userId }) {
  ...
  const [form,     setForm]     = useState({desc:"",amount:"",type:"Expense",cat:"Grocery",imgs:[]});
  const [open,     setOpen]     = useState(false);
  ...
  const add = ()=>{
    ...
    S([{...form,amount:parseFloat(form.amount),id:Date.now(),date:new Date().toLocaleDateString()},...txns]);
    setForm({desc:"",amount:"",type:"Expense",cat:"Grocery",imgs:[]}); setOpen(false);
  };
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",gap:12,minHeight:0}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,flexShrink:0}}>
        {[["Balance",inc-exp,(inc-exp)>=0?GRN:C.r],["Income",inc,GRN],["Expenses",exp,C.r]].map(([l,v,col])=>(
          <div key={l} style={{background:"#fff",borderRadius:14,border:"1.5px solid "+C.bd,padding:"13px 14px"}}>
            <div style={{fontSize:10,color:C.g,fontWeight:700,marginBottom:4,letterSpacing:1,textTransform:"uppercase"}}>{l}</div>
            <div style={{fontSize:17,fontWeight:800,color:col,fontFamily:"'Nunito',sans-serif"}}>{"$"+Math.abs(v).toFixed(2)}</div>
          </div>
        ))}
      </div>
      <button onClick={()=>setOpen(!open)} style={{...btn(),borderRadius:20,padding:"8px 18px",alignSelf:"flex-start"}}>+ Add</button>
      {open && (
        <div style={box}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:10}}>
            ...
            <select style={inp} value={form.type} onChange={e=>setForm({...form,type:e.target.value})}><option>Expense</option><option>Income</option></select>
            <select style={inp} value={form.cat}  onChange={e=>setForm({...form,cat:e.target.value})}>{MCATS.map(c=><option key={c.id}>{c.id}</option>)}</select>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={add} style={{...btn(),flex:1}}>Save</button>
            <button onClick={()=>setOpen(false)} style={{...btn("#fff",C.k),border:"1.5px solid "+C.bd,padding:"10px 16px"}}>Cancel</button>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
        {txns.length===0 && <div style={{textAlign:"center",color:C.g,padding:40,fontSize:14}}>No transactions yet.</div>}
        {txns.map(t=>{
          const catObj=MCATS.find(c=>c.id===t.cat)||MCATS[MCATS.length-1];
          const expanded=expandId===t.id, imgs=t.imgs||[];
          return(
            <div key={t.id} style={{background:"#fff",border:"1.5px solid "+C.bd,borderRadius:14,overflow:"hidden",boxShadow:"0 1px 8px rgba(0,0,0,.05)"}}>
              <div onClick={()=>setExpandId(expanded?null:t.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 15px",cursor:"pointer"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:14,color:C.k}}>{t.desc}</div>
                  <div style={{fontSize:12,color:C.g,marginTop:2,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                    <span style={{background:catObj.color+"18",color:catObj.color,borderRadius:20,padding:"1px 8px",fontWeight:700,fontSize:11}}>{catObj.icon} {catObj.id}</span>
                    <span>{t.date}</span>
              ...
              {expanded && (
                <div style={{borderTop:"1px solid #f5f0ee",padding:"12px 15px 14px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    ...
                    <button onClick={()=>S(txns.filter(x=>x.id!==t.id))} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:12,fontWeight:700}}>Delete ✕</button>
                  </div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                    {imgs.map((img,i)=>(
                      <div key={img.id} style={{position:"relative",borderRadius:8,overflow:"hidden",border:"1.5px solid "+C.bd,flexShrink:0}}>
                        ...
                        <button onClick={()=>removeImg(t.id,img.id)} style={{position:"absolute",top:2,right:2,width:16,height:16,borderRadius:"50%",background:"rgba(0,0,0,.65)",border:"none",color:"#fff",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>x</button>
                      </div>
                    ))}
          ...
          {tab === "appearance" && <>
            ...
            <div style={{background:bg2,borderRadius:14,padding:"14px 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[["🎨 Chalk","#6b8c52"],["🔑 Logins","#2C5F9E"],["📋 Lists","#B85C00"],["📅 Calendar",C.r],["💰 Money","#1B6B35"],["❤️ Health","#B5174A"]].map(([l,col])=>(
              ...
              <div style={{position:"absolute",top:38,right:0,background:D.headerBg,border:"1.5px solid "+D.border,borderRadius:14,boxShadow:"0 8px 32px rgba(0,0,0,.18)",zIndex:200,minWidth:180,overflow:"hidden"}}>
                ...
                {[
                  ...
                  <button key={l} onClick={fn} style={{display:"block",width:"100%",padding:"12px 16px",background:"none",border:"none",textAlign:"left",fontSize:14,color:l==="Sign Out"?C.r:D.text,fontWeight:l==="Sign Out"?700:500,cursor:"pointer",fontFamily:"inherit",borderTop:"1px solid "+D.border}}>
                    {l==="🌙  Dark Mode" ? (dark?"☀️  Light Mode":"🌙  Dark Mode") : l}
                  </button>
                ))}
              </div>
            )}
