  ...
  return (
    <div style={{minHeight:"100svh",background:"linear-gradient(160deg,#C8220A 0%,#8B1507 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 16px",fontFamily:"'DM Sans',sans-serif",overflowY:"auto"}}>
      ...
      <div style={{width:"100%",maxWidth:400}}>
        ...
        <div style={{background:"rgba(0,0,0,.45)",border:"1px solid rgba(255,255,255,.1)",borderRadius:24,padding:"28px 24px",backdropFilter:"blur(10px)"}}>
          ...
          {mode === "reset" && (
            <div style={{marginBottom:22}}>
              <button onClick={()=>{setMode("in");setErr("");setInfo("");}} style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",fontSize:13,cursor:"pointer",fontFamily:"inherit",padding:0,display:"flex",alignItems:"center",gap:4}}>
              <button onClick={()=>{setMode("in");setErr("");setInfo("");}} style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",fontSize:13,cursor:"pointer",fontFamily:"inherit",padding:0,display:"flex",alignItems:"center",gap:4}}>
            ...
            <div style={{marginTop:18,background:"rgba(0,0,0,.25)",borderRadius:12,padding:"14px 16px"}}>
              ...
              {showDisc && (
                <div style={{marginTop:12,padding:"12px 14px",background:"rgba(0,0,0,.3)",borderRadius:10,fontSize:11,color:"rgba(255,255,255,.6)",lineHeight:1.7}}>
                  <div style={{fontWeight:800,color:"rgba(255,255,255,.9)",fontSize:12,marginBottom:6}}>
          ...
          <button onClick={go} disabled={loading} style={{width:"100%",marginTop:20,padding:"15px",borderRadius:14,border:"none",background:C.r,color:"#fff",fontWeight:800,fontSize:16,fontFamily:"'Nunito',sans-serif",cursor:"pointer",opacity:loading?0.6:1,boxShadow:"0 4px 20px rgba(200,34,10,.4)"}}>
            {loading ? "Please wait..." : mode==="reset" ? "Send Reset Email" : mode==="in" ? "Sign In 
          </button>
          {mode === "in" && (
        ...
        <p style={{textAlign:"center",color:"rgba(255,255,255,.25)",fontSize:11,marginTop:16,letterSpacing:1}}>Your data is saved to the cloud 
      </div>
    </div>
  );
}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",borderRadius:10,background:"#6b8c52",boxShadow:"inset 0 0 60px rgba(0,0,0,.25)"}}>
      ...
      <div style={{flexShrink:0,background:"rgba(0,0,0,.22)",borderBottom:"2px solid rgba(0,0,0,.3)",padding:"8px 14px 9px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
          ...
          <span style={{color:"rgba(255,255,255,.35)",fontSize:12}}>{saving?" saving...":"
          {mode==="draw" && <button onClick={eraseLast} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.15)",borderRadius:6,color:"rgba(255,255,255,.65)",padding:"3px 10px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
        ...
        {fil.map(l=>(
          <div key={l.id} style={card}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
              ...
              <button onClick={()=>S(data.filter(x=>x.id!==l.id))} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:13,padding:4}}>
            </div>
          </div>
        ))}
        ...
        {lists.map(l=>(
          <button key={l.id} style={{...pill(active===l.id),display:"flex",alignItems:"center",gap:4}} onClick={()=>setActive(l.id)}>
            ...
            {!BASE_LISTS.some(b=>b.id===l.id) && (
              <span onClick={e=>{e.stopPropagation();removeList(l.id);}} style={{opacity:.6,fontSize:9,marginLeft:1,cursor:"pointer"}}>
            )}
          </button>
        ))}
      ...
      {showN && (
        <div style={box}>
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>setShowN(false)} style={{...btn("#fff",C.k),border:"1.5px solid "+C.bd,padding:"10px 14px"}}>
            <button onClick={()=>setShowN(false)} style={{...btn("#fff",C.k),border:"1.5px solid "+C.bd,padding:"10px 14px"}}>
          </div>
        </div>
      )}
        ...
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
          {al.items.map(it=>(
            <div key={it.id} style={{display:"flex",alignItems:"center",gap:11,padding:"12px 14px",background:"#fff",borderRadius:14,border:"1.5px solid "+C.bd,opacity:it.done?.5:1}}>
              <button onClick={()=>toggle(it.id)} style={{width:24,height:24,borderRadius:8,border:"2px solid "+(it.done?C.r:C.bd),background:it.done?C.r:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:13,flexShrink:0,cursor:"pointer"}}>{it.done?"
              <span style={{flex:1,fontSize:14,textDecoration:it.done?"line-through":"none",color:it.done?C.g:C.k}}>{it.text}</span>
              <button onClick={()=>del(it.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:13}}>
            </div>
          ))}
      ...
      {sel ? (
        <div style={{display:"flex",flexDirection:"column",gap:10,flex:1,minHeight:0}}>
          <div style={{background:C.r,borderRadius:14,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            ...
            <button onClick={()=>S({...evts,[dk(sel)]:note})} style={{background:"rgba(255,255,255,.2)",border:"none",borderRadius:10,padding:"8px 16px",color:"#fff",fontWeight:700,cursor:"pointer",fontFamily:"inherit",fontSize:14}}>Save 
          </div>
            ...
            <div onClick={()=>setExpandId(expanded?null:t.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 15px",cursor:"pointer"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:C.g,marginTop:2,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  ...
                  <span>{t.date}</span>
                  {imgs.length>0&&<span style={{color:C.r,fontWeight:700}}>
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0}}>
 {imgs.length}</span>}
                <div style={{fontWeight:800,fontSize:15,color:txColor}}>{(isIncome?"+":"-")+"$"+t.amount.toFixed(2)}</div>
                <div style={{fontSize:10,color:expanded?"#fff":C.g,background:expanded?txColor:txColor+"18",borderRadius:20,padding:"2px 8px",fontWeight:700}}>{expanded?"
              </div>
            </div>
            {expanded && (
              <div style={{borderTop:"1px solid #f5f0ee",padding:"12px 15px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  ...
                  <button onClick={()=>{S(allTxns.filter(x=>x.id!==t.id));setExpandId(null);}} style={{background:C.r,border:"none",cursor:"pointer",color:"#fff",fontSize:12,fontWeight:700,borderRadius:8,padding:"4px 10px"}}>Delete 
                </div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                  {imgs.map((img,i)=>(
                    <div key={img.id} style={{position:"relative",borderRadius:8,overflow:"hidden",border:"1.5px solid "+C.bd,flexShrink:0}}>
                      ...
                      <button onClick={()=>removeImg(t.id,img.id)} style={{position:"absolute",top:2,right:2,width:18,height:18,borderRadius:"50%",background:"rgba(0,0,0,.7)",border:"none",color:"#fff",fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
                    </div>
                  ))}
      ...
      <div style={{...card,flexDirection:"column",alignItems:"stretch",padding:0,overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,padding:"13px 15px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
            <button onClick={()=>setExpandId(expanded?null:item.id)} style={{background:expanded?C.rl:"#f5f5f5",border:"1.5px solid "+(expanded?C.rm:C.bd),borderRadius:8,padding:"3px 9px",fontSize:11,fontWeight:700,color:expanded?C.r:C.g,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{cnt>0?"
            <button onClick={onDel} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:13,padding:2}}>
          </div>
        </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:500,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:bg,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,maxHeight:"88vh",overflowY:"auto",paddingBottom:32,boxShadow:"0 -8px 40px rgba(0,0,0,.3)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px 16px"}}>
          ...
          <button onClick={onClose} style={{background:bg2,border:"none",borderRadius:10,width:32,height:32,cursor:"pointer",fontSize:16,color:sub}}>
        </div>
        {/* Tabs */}
          ...
          {tab === "appearance" && <>
            ...
            <div style={{background:bg2,borderRadius:14,padding:"14px 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[[" Chalk","#6b8c52"],[" Logins","#2C5F9E"],[" Lists","#B85C00"],["
...
export default function Script() {
  ...
  Dark Mode", ()=>{setDark(!dark);setShowMenu(false);}],
                  ...
                  <button key={l} onClick={fn} style={{display:"block",width:"100%",padding:"12px 16px",background:"none",border:"none",textAlign:"left",fontSize:14,color:l==="Sign Out"?C.r:D.text,fontWeight:l==="Sign Out"?700:500,cursor:"pointer",fontFamily:"inherit",borderTop:"1px solid "+D.border}}>
                    {l==="
  Dark Mode" ? (dark?"  Light Mode":"  Dark Mode") : l}
                  </button>
                ))}
              </div>
            )}
